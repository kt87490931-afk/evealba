...
## 6.5 Phase 5.5: GitHub Actions 자동 배포

### 목표

- `main` 브랜치에 push 시 서버 자동 배포 (git pull + migration)

### 단계별 액션

1. **GitHub Secrets 등록**
   - 저장소 → Settings → Secrets and variables → Actions → New repository secret
   - Name: `EVEALBA_SSH_KEY`
   - Value: 서버 SSH 비밀키 전체 내용 (`-----BEGIN ... -----END ...`)

2. **SSH 키 준비**
   - `C:\Users\DOGE\.ssh\evealba_key` 또는 `id_rsa` 중 서버 접속에 사용하는 키의 **비밀키** 전체를 복사
   - GitHub Secret에 붙여넣기 (공개키 아님, 비밀키)

3. **동작**
   - `git push origin main` 시 `.github/workflows/deploy.yml` 실행
   - 서버 `188.166.179.115` SSH 접속 → `/var/www/evealba`에서 `git fetch` + `git reset --hard origin/main` + `php run_migration.php`
...
| 항목 | 방법 |
|------|------|
| 워크플로 동작 | push 후 Actions 탭에서 Deploy to Server 실행 확인 |
| 실패 시 | EVEALBA_SSH_KEY 값·형식 확인, 서버에서 `authorized_keys`에 GitHub Actions용 공개키 필요 없음 (비밀키로 접속) |
