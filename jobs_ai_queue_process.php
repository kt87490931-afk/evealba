...
for ($i = 0; $i < $limit; $i++) {
    ...
    $employ_type = isset($jr_data['employ-type']) ? trim($jr_data['employ-type']) : (isset($jr_data['employ_type']) ? trim($jr_data['employ_type']) : '');
    $amenity = isset($jr_data['amenity']) ? (is_array($jr_data['amenity']) ? $jr_data['amenity'] : array_filter(explode(',', (string)$jr_data['amenity']))) : array();
    $keyword = isset($jr_data['keyword']) ? (is_array($jr_data['keyword']) ? $jr_data['keyword'] : array_filter(explode(',', (string)$jr_data['keyword']))) : array();
    $mbti_prefer = isset($jr_data['mbti_prefer']) ? (is_array($jr_data['mbti_prefer']) ? $jr_data['mbti_prefer'] : array_filter(explode(',', (string)$jr_data['mbti_prefer']))) : array();
    $formData = array(
        'nickname' => isset($jr_data['job_nickname']) ? trim($jr_data['job_nickname']) : $jr['jr_nickname'],
        'company' => isset($jr_data['job_company']) ? trim($jr_data['job_company']) : (isset($jr['jr_company']) ? $jr['jr_company'] : ''),
        ...
        'qualify' => isset($jr_data['desc_qualify']) ? trim($jr_data['desc_qualify']) : '',
        'details' => isset($jr_data['desc_extra']) ? trim($jr_data['desc_extra']) : '',
