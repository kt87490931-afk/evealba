    ...
    if ($tb_check && sql_num_rows($tb_check)) {
        ...
        while ($row = sql_fetch_array($result)) {
            ...
            $list[] = array(
                ...
                'jump_remain' => isset($row['jr_jump_remain']) ? (int)$row['jr_jump_remain'] : 0,
                'remaining' => $remaining,
                'ad_labels' => $ad_labels,
...
<?php if ($row['status'] === 'ongoing') { ?>
            <span class="mc-bottom-right">
              <span class="mi-jump-num" id="mc-jump-<?php echo $row['jr_id']; ?>"><?php echo number_format($row['jump_remain']); ?>회</span>
              <button type="button" class="btn-jump-inline" onclick="event.preventDefault();event.stopPropagation();doListJump(<?php echo $row['jr_id']; ?>);" <?php echo $row['jump_remain'] <= 0 ? 'disabled' : ''; ?>>⚡점프</button>
            </span>
<?php } ?>
          </div>
        </div>
...
function doListJump(jrId){
  var btn=document.getElementById('btn-jump-'+jrId);
